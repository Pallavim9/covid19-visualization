...
function viz() {
    d3.csv(nationalDataURL, d => ({
        ...
        // Assume these are the dates where surges start
        const surgeDates = [
            { date: parseTime('2020-11-8'), label: 'First Surge' },
            { date: parseTime('2021-12-26'), label: 'Second Surge' }
        ];

        const annotations = surgeDates.map(d => ({
            note: { label: d.label, align: "middle", wrap: 200 },
            x: x1(d.date),
            y: y1(nationalData.find(data => data.date.getTime() === d.date.getTime()).cases),
            dy: -30,
            dx: 0
        }));
        ...
        function updateStateTimeChart(selectedDate, selectedState, callback) {
            d3.csv(stateDataURL, d => ({
                ...
                const restartButton = d3.select("#chart3").append("button")
